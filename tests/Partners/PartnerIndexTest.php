...
class PartnerIndexTest extends TestCase
{
    ...
    {
        $response = Partners::index(perPage: 101);
        $this->assertSame(25, $response['per_page']);
        $response = Partners::index(perPage: 0);
        $this->assertSame(25, $response['per_page']);
    }

    public function testItCanListPartners(): void
    {
        $partner = $this->partnerFactory();

        $created = Partners::store($partner);

        $response = Partners::index(query: $partner->getName())['data'][0];
